...
for (my $count =0 ; $count < 16; $count++) {
	...
	($webpage[$count*2+4]) = $arr[$count]."<br>";
	}
	else {
	#($webpage[$count*2+4]) = ($count+1)."   ";
	($webpage[$count*2+4]) = $arr[$count]."   ";
	}
}
$webpage[100] = "\r\n</h4>\r\n</body>\r\n</html>";
...
open ("OUT1", ">/home/m/mcs51.h16.ru/WWW/receive.html"); 
my $favorite = "0";
...
for (my $count1=0; $count1<16; $count1++){ 
	...
	if ($favorite == $arr[$count1]) {
		$webpage[$count1*2+3] = '<font color="#CC0000">';
		$webpage[$count1*2+5] = "</font>";
	}
	}
}
